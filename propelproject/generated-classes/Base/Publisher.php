...
abstract class Publisher implements ActiveRecordInterface
{
    ...
    public function setBookPublishers(Collection $bookPublishers, ConnectionInterface $con = null)
    {
        ...
        foreach ($bookPublishersToDelete as $bookPublisherRemoved) {
            $bookPublisherRemoved->setPublisher(null);
        }
     ...
     * @return ObjectCollection|ChildBookPublisher[] List of ChildBookPublisher objects
     */
    public function getBookPublishersJoinBook(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildBookPublisherQuery::create(null, $criteria);
     ...
     * @return ChildPublisher The current object (for fluent API support)
     */
    ...
    protected function doAddBook(ChildBook $book)
    {
        ...
        if (!$book->isPublishersLoaded()) {
            $book->initPublishers();
            $book->getPublishers()->push($this);
        } elseif (!$book->getPublishers()->contains($this)) {
            $book->getPublishers()->push($this);
        }
    ...
    {
        if ($this->getBooks()->contains($book)) { $bookPublisher = new ChildBookPublisher();

            $bookPublisher->setBook($book);
            if ($book->isPublishersLoaded()) {
                //remove the back reference if available
                $book->getPublishers()->removeObject($this);
            }
